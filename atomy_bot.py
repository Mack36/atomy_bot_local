...
import re
from config import WEBHOOK_PATH, WEBAPP_HOST, WEBAPP_PORT, API_TOKEN, WEBHOOK_URL, ADMINS, TESTENV, PUBLIC_IP
import texts
import shelve
import ssl
import subprocess
...
statestmp = {}
gencmd = 'sudo cd /home/ec2-user/python/atomy_bot; sudo openssl req -newkey rsa:2048 -sha256 -nodes -keyout myprivate.key -x509 -days 365 -out mypublic.pem -subj \"/C=UZ/ST=Tashkent/L=Tashkent/O=Atomy Shokhsaroy/CN=' + PUBLIC_IP + '\"'
result = subprocess.run([gencmd], shell=True)
print(result)
...
async def main_menu(message: types.Message):
    ...
    # kbr = types.ReplyKeyboardRemove()
    # await bot.send_message(message.chat.id, reply_markup=kbr)
...
async def proc_contact(message: types.Message):
    ...
    kbr = types.ReplyKeyboardRemove()
    await message.answer(texts.partner_finish[lang], reply_markup=kbr)
